...
namespace RedactApplication.Controllers
{
    public class CommandesController : Controller
    {
        ...
        public async Task<ActionResult> GenerateMotCle(COMMANDEViewModel commandeVm)
        {
            ...
            int guide_id = GetGuideID();
            string url = "https://yourtext.guru/api/guide/"+guide_id;
           
            var guide = await GetGuideAssetAsync(guide_id);
            Session["keys"] = guide;
        ...
        private async Task<string> GetGuideAssetAsync(int guide_id)
        {
            string url = "https://yourtext.guru/api/guide/" + guide_id;
        ...
        private async System.Threading.Tasks.Task GenerateCommandeKeysAsync(CancellationToken cancellationToken)
        {
            try
            {
                ...
                listeDataCmde = listeDataCmde.Where(x => x.date_livraison >= startOfMonth && x.date_livraison <= lastDay).ToList();
                ...
                if (listeDataCmde.Count() > 0)
                {
                    ...
                    foreach (var cmd in listeDataCmde)
                    {
                        ...
                        if (string.IsNullOrEmpty(commande.guide_id))
                        {
                        ...
                        else
                        {
                            ...
                            {
                                //Lancer une commande de guide              
                                string url = "https://yourtext.guru/api/guide/" + commande.guide_id;
        ...
        public JsonResult LoadScoreByContent(string contenu,string idCommande)
        {
            contenu = System.Net.WebUtility.HtmlDecode(contenu);

            var res =  new Worker().GetScoringAsync(idCommande, contenu);            
        ...
        public ActionResult CreateNewCommande(COMMANDEViewModel model, FormCollection collection)
        {
            ...
            if (commande <= 0)
            {
               ...
               // newcommande.commandeThemeId = selectedThemeId;
                ...
                try
                {
                    ...
                    if(res > 0)
                    {
                        bool sendsms = Session["notifSms"] != null ? true : false;   
                        ...
                        if (Request.Url != null)
                        {
                            ...
                            if (newcommande.REDACTEUR != null)
                            {
                                ...
                                bool isSendMail =  MailClient.SendMail(newcommande.REDACTEUR.userMail, mailBody.ToString(), "Media click App - nouvelle commande.");
                                if (isSendMail)
                                {
        ...
        public ActionResult CommandeTransfert(Guid idCommande, COMMANDEViewModel model)
        {
            ...
            if (Request.Url != null)
            {
                ...
                if (newcommande.REDACTEUR != null)
                {
                    ...
                    bool isSendMail =  MailClient.SendMail(newcommande.REDACTEUR.userMail, mailBody.ToString(), "Media click App  - tranfert commande.");
                    if (isSendMail)
                    {
                        newcommande.commandeToken = newcommande.commandeId;
                        newcommande.dateToken = DateTime.Now;
        ...
        public ActionResult RelanceSMS(Guid hash)
        {
            ...
            if (commande.REDACTEUR != null)
            {
                string redactNumber = "+261" + commande.REDACTEUR.redactPhone.Trim();
                var to = new PhoneNumber(redactNumber);
                var message = MessageResource.Create(
                    to,
                    @from: new PhoneNumber(phonenumber),
